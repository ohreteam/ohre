...
class MethodStringLiteralRegionIndex(BaseRegion):
    def __init__(self, buf, pos: int, method_string_literal_region_idx_size: int):
        ...
        if (method_string_literal_region_idx_size <= 0 or method_string_literal_region_idx_size == op._get_uint32_t_max()):
            return
    ...
    def __str__(self):
        ...
        out = f"MethodStringLiteralRegionIndex({hex(len(self.offsets))}): [{hex(self.pos_start)}/{hex(self.pos_end)}] {out_offsets}"
        return out
